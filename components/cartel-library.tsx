...
import type { AnalysisStatus, AnalyzedCartel } from "@/data/territorial";
import { analysisColors, analysisLabels } from "@/data/territorial";

/** Resultados geométricos verificables del análisis territorial. */
const STATUS_FILTERS: AnalysisStatus[] = ["dentro_corredor", "cerca_lugar_permitido", "fuera_zona_permitida"];
...
export function CartelLibrary({ carteles, onLocate }: { carteles: AnalyzedCartel[]; onLocate: (cartel: AnalyzedCartel) => void }) {
  ...
  const filtered = useMemo(() => carteles.filter(item => {
    const visualStatus = item.properties.analysisStatus;
    if (statuses.length > 0 && !statuses.includes(visualStatus)) return false;
    return `${item.properties.name || ""} ${item.properties.nearestCorridor || ""} ${analysisLabels[visualStatus]}`.toLowerCase().includes(query.toLowerCase());
  ...
  const toggleStatus = (status: AnalysisStatus) => {
  ...
  return <section id="carteles" className="section-block">
    <div className="section-heading"><div><span className="section-kicker">Diagnóstico territorial</span><h2>Carteles analizados</h2><p>Resultados geométricos para orientar una revisión administrativa posterior.</p></div><span className="text-xs font-bold text-slate-400">{filtered.length} registros</span></div>
    ...
    <div className="mb-6 flex flex-wrap items-center gap-1.5" role="group" aria-label="Filtrar por diagnóstico territorial">
      ...
      {STATUS_FILTERS.map(status => {
        ...
        return <button key={status} type="button" aria-pressed={active} onClick={() => toggleStatus(status)} className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-2 text-[9px] font-extrabold transition ${active ? "border-municipal-700 bg-municipal-700 text-white shadow-sm" : "border-slate-200 bg-white text-slate-600 hover:border-municipal-300 hover:text-municipal-700"}`}>
          <i className="size-2 rounded-full ring-1 ring-white/70" style={{ background: analysisColors[status] }}/>{analysisLabels[status]}
        </button>;
      })}
    </div>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map(item => {
      ...
      return <article key={String(item.properties.id)} className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 transition duration-200 hover:-translate-y-0.5 hover:border-municipal-500/40 hover:shadow-card">
        ...
        <span className="mt-3 w-fit rounded-full px-2.5 py-1 text-[8px] font-extrabold uppercase tracking-wider text-white" style={{ background: analysisColors[visualStatus] }}>{analysisLabels[visualStatus]}</span>
        ...
        {status === "fuera_zona_permitida" && <div className="mt-3 flex items-center gap-2 rounded-lg bg-red-50 px-3 py-2 text-[9px] font-bold text-red-700"><ShieldCheck size={13}/>Fuera de las áreas analizadas · requiere evaluación administrativa</div>}
