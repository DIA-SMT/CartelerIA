...
export function CartelLibrary({ carteles }: { carteles: AnalyzedCartel[] }) {
  ...
  return <section id="carteles" className="section-block">
    ...
    <label className="filter-input mb-6 max-w-xl border border-slate-200 bg-white"><Search size={16}/><input value={query} onChange={event => { setQuery(event.target.value); setPage(1); }} aria-label="Buscar carteles analizados" placeholder="Buscar dirección, corredor o estado..."/></label>
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map(item => {
      ...
      return <article key={String(item.properties.id)} className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 transition duration-200 hover:-translate-y-0.5 hover:border-municipal-500/40 hover:shadow-card">
        ...
        <footer className="mt-auto grid grid-cols-2 gap-2 border-t border-slate-100 pt-4"><a href={maps} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center justify-center gap-1 rounded-lg bg-slate-50 px-2 text-[9px] font-bold text-slate-600 transition hover:bg-municipal-50 hover:text-municipal-700">Google Maps <ExternalLink size={11}/></a><a href={street} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center justify-center gap-1 rounded-lg bg-municipal-700 px-2 text-[9px] font-bold text-white transition hover:bg-municipal-900">Street View <ExternalLink size={11}/></a></footer>
      </article>;
    })}</div>
