...
export function StateChangeApprovals({
  ...
  return (
    <div className="space-y-2">
      ...
      {loadError && (
        <p role="alert" className="flex items-start gap-1.5 rounded-lg border border-red-200 bg-red-50 px-2.5 py-2 text-micro font-semibold leading-4 text-red-700">
            ...
            {isAdmin && (
              <>
                ...
                <div className="mt-2.5 flex gap-2">
                  <button type="button" disabled={Boolean(loadError) || busy || note.trim().length < 5} onClick={() => resolve(request, true)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-green-700 px-3.5 text-micro font-extrabold text-white transition duration-fast hover:bg-green-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-600 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                    {busy ? <Loader2 size={12} className="animate-spin"/> : <CheckCircle2 size={12}/>}
                    Aprobar
                  </button>
                  <button type="button" disabled={Boolean(loadError) || busy || note.trim().length < 5} onClick={() => resolve(request, false)} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3.5 text-micro font-extrabold text-red-700 transition duration-fast hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                    <XCircle size={12}/>
                    Rechazar
                  </button>
                </div>
                {failedId === request.id && <p role="alert" className="mt-1.5 text-micro font-semibold text-red-700">No se pudo resolver la solicitud.</p>}
              </>
            )}
          </div>
        );
      ...
      {resolved.length > 0 && (
        <ul className="space-y-1">
          {resolved.map((request) => (
            <li key={request.id} className="rounded-lg bg-slate-50 px-2.5 py-2 text-micro text-slate-500">
