...
export default function CarteleriaMap({ carteles, corridors, allowedPlaces, selected, onSelect }: {
  ...
  return <MapContainer center={[-26.8304, -65.2145]} zoom={13} minZoom={12} maxZoom={18} className="h-full w-full" zoomControl={false} scrollWheelZoom>
    ...
    {carteles.map(cartel => {
      ...
      const visualStatus = cartel.properties.analysisStatus;
      return <CircleMarker key={String(cartel.properties.id)} center={[latitude, longitude]} radius={active ? 11 : 7} pathOptions={{ color: "white", weight: active ? 4 : 2, fillColor: analysisColors[visualStatus], fillOpacity: 1 }} eventHandlers={{ click: () => onSelect(cartel) }}>
        <Tooltip direction="top" offset={[0, -7]}><strong>{cartel.properties.name || "Cartel relevado"}</strong><br/>{analysisLabels[visualStatus]}<br/>{Math.round(Number(cartel.properties.distanceToCorridorM || 0))} m del corredor más cercano</Tooltip>
      </CircleMarker>;
    })}
